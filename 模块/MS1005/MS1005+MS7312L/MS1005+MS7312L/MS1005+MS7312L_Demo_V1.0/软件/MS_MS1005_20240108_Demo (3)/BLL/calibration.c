...
const RANGETypedef APDCalibration_IntensityRange  = {450 , 480};  //调整增益时目标靶的强度范围
...
//    SendReceive_Command(0x61);  //150mm靶归位
   ...
   SendReceive_Command(0x4C);  //测500mm强反强度
    ...
    while(3 != Calibration_Step)
    {
        ...
        switch((u8)Compare_Range(point.intensity, TargetRange))
        {
            case LOWSIDE:   //偏低 
                ...
                break;
            case HIGHSIDE:  //偏高
                if ( point.intensity > StrongTarget_Reflect-10)//在透明区域
    ...
    Flash_Write_Byte(COMPENSATION_DATA_ADDRESS, (uint8_t *)Compensation, COMPENSATION_SIZE*2);
	...
	Flash_Write_Byte(CALIBRATION_DATA_ADDRESS, (uint8_t *)&cal_param.flag, sizeof(CAL_Param));
