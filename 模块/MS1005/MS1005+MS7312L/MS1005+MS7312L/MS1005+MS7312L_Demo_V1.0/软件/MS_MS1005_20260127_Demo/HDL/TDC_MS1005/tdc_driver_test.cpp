...
typedef struct {
	...
	uint32_t RESERVED2    : 1; //11    必须设置为默认值
	...
	uint32_t DOUBLE_RES   : 1; //18    双精度测量开启（当开启双精度时，只能使用STOP1通道，STOP2 通道无效）: 0 = 关闭, 1 = 开启
	uint32_t START_CLKHS  : 1; //19    设置晶振的开启和关闭（晶振关闭后可以进行非校准模式测试）
	uint32_t DIV_CLKHS    : 2; //20:21 设置 CLKHS 高速参考时钟的分频因数
	uint32_t SEL_TIMO     : 2; //22:23 溢出时间选择（使该功能有效必须EN_SEL_TIMO=1）另外该溢出时间仅和高速时钟相关
...
int main(void)
{
	uint32_t config_res_value = 0x225A6490;
	uint32_t test_value = 0x00;
	uint32_t test_value2 = 0x00;
	TDC_CONFIG_RES tdc_config_res = TDC_Config_Res_Parse(config_res_value);
	TDC_CONFIG_RES test_res;

	memcpy(&test_res, &config_res_value, 4);
	memcpy(&test_value2, &test_res, 4);
	test_value = TDC_Config_Res_Package(tdc_config_res);
	//tdc_config_res.EN_INT = 2;
	printf("sizeof(TDC_CONFIG_RES) = %d\n", sizeof(TDC_CONFIG_RES));
	printf("test_value  = 0x%lx\n", test_value);
	printf("test_value2 = 0x%lx\n", test_value2);
