...
TDC_CONFIG_RES TDC_Config_Res_Parse(uint32_t res_value)
{
    ...
    config_res.RFEDGE1      = REG_READ_BIT(res_value, 0 , n)；
    config_res.RFEDGE2      = REG_READ_BIT(res_value, 1 , n)；
    config_res.RESERVED1    = REG_READ_BIT(res_value, 2 , n)；
    config_res.EN_INT       = REG_READ_BIT(res_value, 4 , n)；
    config_res.EN_SEL_TIMO  = REG_READ_BIT(res_value, 7 , n)；
    config_res.NEG_START    = REG_READ_BIT(res_value, 8 , n)；
    config_res.NEG_STOP1    = REG_READ_BIT(res_value, 9 , n)；
    config_res.NEG_STOP2    = REG_READ_BIT(res_value, 10, n)；
    config_res.RESERVED2    = REG_READ_BIT(res_value, 11, n)；
    config_res.NO_CAL_AUTO  = REG_READ_BIT(res_value, 12, n)；
    config_res.CALIBRATE    = REG_READ_BIT(res_value, 13, n)；
    config_res.SEL_TSTO2    = REG_READ_BIT(res_value, 14, n)；
    config_res.EN_FAST_INIT = REG_READ_BIT(res_value, 16, n)；
    config_res.EN_ERR_VAL   = REG_READ_BIT(res_value, 17, n)；
    config_res.DOUBLE_RES   = REG_READ_BIT(res_value, 18, n)；
    config_res.START_CLKHS  = REG_READ_BIT(res_value, 19, n)；
    config_res.DIV_CLKHS    = REG_READ_BIT(res_value, 20, n)；
    config_res.SEL_TIMO     = REG_READ_BIT(res_value, 22, n)；
    config_res.HITIN1       = REG_READ_BIT(res_value, 24, n)；
}
