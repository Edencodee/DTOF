...
void byte_swap(uint8_t *pData1, uint8_t *pData2, size_t stSize)
{
	...
	for (i = 1; hasSwap && i < nmemb; i++) {
		...
		for (j = 0; j < nmemb - i; j++) {
			...
			if (compare(pThis, pNext) > 0) {
				hasSwap = 1;
				byte_swap((uint8_t *)pThis, (uint8_t *)pNext, size);
			}
		}
	}
}
									...
									COMPARE compare)
{
	...
	for (i = 1; hasSwap && i < nmemb; i++) {
		...
		for (j = 0; j < nmemb - i; j++) {
			...
			if (compare(pThis, pNext) > 0) {
				hasSwap = 1;
				byte_swap((uint8_t *)pThis - (uint8_t *)pos, (uint8_t *)pNext - (uint8_t *)pos, struct_size);
			}
		}
	}	
}
