...
void Flash_ReadWrite_Test(void)
{
	...
	Flash_Write(COMPENSATION_DATA_ADDRESS, test_buf, test_size);
	delay_us(10);

	Flash_Read(COMPENSATION_DATA_ADDRESS, read_buf, test_size);

	for (uint8_t i = 0; i < test_size; i++) {
		printf("test[%d] = %d, read[%d] = %d", i, test_buf[i], i, read_buf[i]);
	...
	if (test_result != 0) {
		printf("flash read write test OK!!!\n");
	} else {
		printf("flash read write test NG!!!\n");
	}
	while (1) {
		;
	}
