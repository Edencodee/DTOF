...
TDC_CONFIG_RES TDC_Config_Res_Parse(uint32_t res_value)
{
    ...
    config_res.RFEDGE1      = REG_READ_BIT(res_value, 0 , 1 : 1;
    config_res.RFEDGE2      = REG_READ_BIT(res_value, 1 , 1 : 1;
    config_res.RESERVED1    = REG_READ_BIT(res_value, 2 , 2 : 2;
    config_res.EN_INT       = REG_READ_BIT(res_value, 4 , 3 : 3;
    config_res.EN_SEL_TIMO  = REG_READ_BIT(res_value, 7 , 1 : 1;
    config_res.NEG_START    = REG_READ_BIT(res_value, 8 , 1 : 1;
    config_res.NEG_STOP1    = REG_READ_BIT(res_value, 9 , 1 9 );
    config_res.NEG_STOP2    = REG_READ_BIT(res_value, 10, 1 10);
    config_res.RESERVED2    = REG_READ_BIT(res_value, 11, 1 11);
    config_res.NO_CAL_AUTO  = REG_READ_BIT(res_value, 12, 1 12);
    config_res.CALIBRATE    = REG_READ_BIT(res_value, 13, 1 : 2;
    config_res.SEL_TSTO2    = REG_READ_BIT(res_value, 14, 2 : 1;
    config_res.EN_FAST_INIT = REG_READ_BIT(res_value, 16, 1 : 1;
    config_res.EN_ERR_VAL   = REG_READ_BIT(res_value, 17, 1 : 1;
    config_res.DOUBLE_RES   = REG_READ_BIT(res_value, 18, 1 : 1;
    config_res.START_CLKHS  = REG_READ_BIT(res_value, 19, 1 : 2
    config_res.DIV_CLKHS    = REG_READ_BIT(res_value, 20, 2 : 2
    config_res.SEL_TIMO     = REG_READ_BIT(res_value, 22, 2 : 4
    config_res.HITIN1       = REG_READ_BIT(res_value, 24, 4 : 4
