...
__IO s_coef_t g_scoef;
uint16_t *g_coef  = (uint16_t*)&g_scoef;
#endif
...
void FLASH_RegWrite (void)
{
	...
	FLASH_UserWrite(REG_ADDR, (uint32_t*)g_reg, kRegCnt / 2);
	__enable_irq();
}
#if APP
void FLASH_ParaWrite (void)
{
	__disable_irq();
	FLASH_UserErase(CTL_SECTOR_NUMBER_1); /* sector 1 */
	FLASH_UserWrite(PARA_ADDR, (uint32_t*)g_para, sizeof(s_para_t) / 4);
	FLASH_UserWrite(REG_ADDR, (uint32_t*)g_reg, kRegCnt / 2);
	__enable_irq();
}

void FLASH_CoefWrite (void)
{
	__disable_irq();
	FLASH_UserErase(CTL_SECTOR_NUMBER_1); /* sector 1 */
	FLASH_UserWrite(PARA_ADDR, (uint32_t*)g_para, sizeof(s_para_t) / 4);
...
void FLASH_RegSetMid (int index)
{
	...
	case kFP32: {			
		fp32_t reg = {.buf16 = { g_reg[index],  g_reg[index + 1]}};				
		fp32_t max = {.buf16 = {regMax[index], regMax[index + 1]}};
		fp32_t min = {.buf16 = {regMin[index], regMin[index + 1]}};	
		if (isnan(reg.valf) || \
...
static int  FLASH_RegsChk (int start, int cnt)
{
	...
	for (int i = start; i < cnt; ) {		
		...
		case kFP32: {			
			fp32_t reg = {.buf16 = { g_reg[i],  g_reg[i + 1]}};				
			fp32_t max = {.buf16 = {regMax[i], regMax[i + 1]}};
			fp32_t min = {.buf16 = {regMin[i], regMin[i + 1]}};	
			if (isnan(reg.valf) || \
			   (reg.valf > max.valf) || (reg.valf < min.valf)) {
				g_reg[i] = regDef[i];
...
void FLASH_RegLoadChk (void)
{
	...
	FLASH_LoadData(REG_ADDR, kRegCnt / 2, (uint32_t)g_reg);
	g_reg[kRegFlag    ] = 0;
...
static void ParaCheck (void) 
{
	...
	/* flag check */
	if ((*(u32_t*)&g_spara.flag).buf16[1] == k5PCali) {
		// ...
	} else if ((*(u32_t*)&g_spara.flag).buf16[1] == kCenterCali) {
		// ...
	} else if ((*(u32_t*)&g_spara.flag).buf16[1] == kNPFit3Cali) {
		// ...
	} else if ((*(u32_t*)&g_spara.flag).buf16[1] == kNPFit4Cali) {
		// ...
	} else if ((*(u32_t*)&g_spara.flag).buf16[1] == kNPIntpCali) {
		// ...
	} else {
		if (isnan(g_spara.yc)) {
//			memcpy((uint8_t*)&g_spara.yc, defPara[0], 16);
		}
	}	
}
...
{
	FLASH_LoadData(PARA_ADDR, sizeof(s_para_t) / 4, (uint32_t)g_para);
	ParaCheck();
}

static void CoefCheck (void) 
{
	for (int i = 0; i < PN; i++) {
		if (isnan(g_scoef.a[i])) {
			// ...
		}
		if (isnan(g_scoef.b[i])) {
			// ...
		}
		if (isnan(g_scoef.c[i])) {
			// ...
		}
		if (isnan(g_scoef.d[i])) {
			// ...
		}
	}
...
void FLASH_CoefLoadChk (void) 
{
	FLASH_LoadData(COEF_ADDR, sizeof(s_coef_t) / 4, (uint32_t)g_coef);
	CoefCheck();
}
#endif
/* -----------------------------------------------------------------------*/
