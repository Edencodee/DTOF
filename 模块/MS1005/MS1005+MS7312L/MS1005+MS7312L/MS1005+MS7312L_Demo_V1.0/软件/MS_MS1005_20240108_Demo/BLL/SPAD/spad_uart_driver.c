...
uint8_t SPAD_Msg_Parsing(uint8_t* buf, SenPF* point, uint16_t data_len, uint16_t workmode)//size_t packet_size)
{
	...
	//校验位正确
	if (buf[pos + packet_size - 1] == Get_Data_XOR(buf + pos + 1, packet_size - 2)) {
...
uint8_t SPAD_Msg_Parsing_V1_1(struct kfifo *fifo, SCAN_MSG_PACKET *scan_msg, uint16_t workmode)
{
	...
	uint8_t bresult = RESET;
	uint16_t packet_size = spad_frame_packet[workmode].packet_size;
	uint32_t data_len = __kfifo_len(fifo);
	uint8_t *buf = NULL;
	SenPF* point = NULL;

	while(data_len >= packet_size) {
		buf = fifo->buffer + (fifo->out & fifo->mask);
		pos = 0;
		...
		if (buf[pos + packet_size - 1] == Get_Data_XOR(buf + 1, packet_size - 2)) {
			...
			((MSG_POINT *)scan_msg->point)[scan_msg->point_cnt].dist = spad_frame_packet[workmode].scan_point->dist;
			scan_msg->point_cnt++;
			bresult = SET;
		}

		fifo->out += (pos + packet_size);
