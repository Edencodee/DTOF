...
uint8_t SPAD_Msg_Parsing(uint8_t* buf, SenPF* point, uint16_t data_len, uint16_t workmode)//size_t packet_size)
{
	...
	if (buf[pos+packet_size-1] == Get_Data_XOR(buf+1, packet_size-2))
	{
		...
		// if (packet_size == PackSize1)
		// {
			// memcpy(&spad_frame_packet_1CH, &buf[pos], packet_size);
// 
			// Get_SPAD_Point_1CH(&spad_frame_packet_1CH, point);
			////point->dist = buf[pos+2]*256+buf[pos+1];
			////point->engy = buf[pos+4]*256+buf[pos+3];
		// }
// 
		// else if (packet_size == PackSize2)
		// {
			// memcpy(&spad_frame_packet_2CH, &buf[pos], packet_size);
			// Get_SPAD_Point_2CH(&spad_frame_packet_2CH, point);
// 
		// }
		//printf("D: %d, I: %d\n", point->dist, point->engy);
...
// static void Get_SPAD_Point_1CH(PacketF_1CH_LiDAR *spad_paceket, SenPF* point)
// {
	// point->dist = spad_paceket->data[0].dist;
	// point->engy = spad_paceket->data[0].engy;
// }
// 
// static void Get_SPAD_Point_2CH(PacketF_2CH_LiDAR *spad_paceket, SenPF* point)
// {
	// int point_choose = 0;
// 
	// if (spad_paceket->data[0].engy > skl100_params.engy_lower_limit 
		// && spad_paceket->data[1].engy > skl100_params.engy_lower_limit)
	// {
		/* 选取距离值较小的点 */		
		// 
		// if (spad_paceket->data[0].dist > spad_paceket->data[1].dist)
		// {
			// point_choose = 1; //
		// }
		// else
	...
	// }
// 
	// else if (spad_paceket->data[0].engy > skl100_params.engy_lower_limit)
	// {
		// point_choose = 0;
	// }
	// else if (spad_paceket->data[1].engy > skl100_params.engy_lower_limit)
	// {
		// point_choose = 1;
	// }
	// else//两个点强度都不满足
...
// 
	// }
// 
	// point->dist = spad_paceket->data[point_choose].dist;
	// point->engy = spad_paceket->data[point_choose].engy;
	// 
// }