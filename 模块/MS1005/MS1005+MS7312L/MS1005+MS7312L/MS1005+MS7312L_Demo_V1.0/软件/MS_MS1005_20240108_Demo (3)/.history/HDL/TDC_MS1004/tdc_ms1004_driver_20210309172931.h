...
typedef struct
{
    ...
    uint32_t RESERVED1    : 1; //4     默认0
    uint32_t STOP1_CNT    : 4; //5:8   STOP1脉冲数
    uint32_t STOP2_CNT    : 4; //9:12  STOP2脉冲数
    uint32_t TDC_OUT      : 1; //13    TDC溢出
    uint32_t TIMEOUT      : 1; //14    时间溢出
    uint32_t RESERVED2    : 1; //15    默认0
    uint32_t STOP2_RESULT : 4; //16:19 STOP2 结果地址指针
    uint32_t RESERVED3    : 4; //20:23 默认0
    uint32_t RESERVED4    : 8; //24:31 默认0
} TDC_STAT_RES;
