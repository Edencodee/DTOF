...
**备注说明：v1.0
**/
uint8_t SPAD_Msg_Parsing(uint8_t* buf, SCAN_MSG_PACKET* scan_msg, uint16_t data_len, uint16_t workmode)//size_t packet_size)
{
	uint16_t pos = 0;
	...
	if (buf[pos+packet_size-1] == Get_Data_XOR(buf+1, packet_size-2))
	{
		...
		scan_msg->point[scan_msg->point_cnt].dist = spad_frame_packet[workmode].scan_point->dist;
